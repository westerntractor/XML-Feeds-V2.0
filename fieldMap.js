...
const MAX_IMAGES_PER_MACHINE = Math.min(
...
function mapImagesRaw(urls) {
  ...
  const { primary, overflow } = splitForWebflowGalleries(urls);

  const out = { "image-gallery": primary.map(toImage) };

  const secondGallery = secondGalleryFieldSlug();

  if (secondGallery) out[secondGallery] = overflow.map(toImage);
...
async function buildMachineFieldsAsync(machine, existingFieldData = null) {
  ...
  const skipImages =
    ...
    feedImagesUnchanged(existingFieldData, sourceUrls) &&

    cmsGalleryLayoutMatches(existingFieldData, sourceUrls);
  ...
  if (isImageSyncDebug()) {
    ...
    debugLog("buildMachineFields", `unique-id=${machine.id}`, {
      ...
      galleryLayoutOk: cmsGalleryLayoutMatches(existingFieldData, sourceUrls),
...
function secondGalleryUrls(fields) {

  const secondSlug = secondGalleryFieldSlug();

  if (!secondSlug) return [];

  const gallery = fields?.[secondSlug];
...
/** Expected Webflow gallery slot counts for a feed image list length. */
function expectedGalleryCounts(imageCount) {

  const n = Math.max(0, Number(imageCount) || 0);

  const capped = Math.min(n, MAX_IMAGES_CAP);

  return {

    primary: Math.min(WEBFLOW_GALLERY_MAX, capped),

    overflow: Math.max(0, capped - WEBFLOW_GALLERY_MAX),
...
/** True when CMS galleries use primary + overflow split (not legacy duplicate full list). */
function cmsGalleryLayoutMatches(fieldData, sourceUrlsOrCount) {

  const count = Array.isArray(sourceUrlsOrCount)

    ? sourceUrlsOrCount.length

    : Number(sourceUrlsOrCount) || 0;

  if (!count) return true;

  const { primary, overflow } = expectedGalleryCounts(count);

  return (

    galleryUrls(fieldData).length === primary &&

    secondGalleryUrls(fieldData).length === overflow
...
function pickImageFieldData(fields) {
  ...
  if (Object.prototype.hasOwnProperty.call(fields, "image-gallery")) {

    out["image-gallery"] = fields["image-gallery"];

  }

  const secondSlug = secondGalleryFieldSlug();

  if (secondSlug && Object.prototype.hasOwnProperty.call(fields, secondSlug)) {

    out[secondSlug] = fields[secondSlug];
...
function imagesSyncEqual(existingFieldData, incomingFields) {
  ...
  if (existingFp && incomingFp) {
    ...
    const inMain = galleryUrls(incomingFields).length;

    const inSecond = secondGalleryUrls(incomingFields).length;

    if (inMain > 0 || inSecond > 0) {

      const layoutOk = cmsGalleryLayoutMatches(
      ...
      if (isImageSyncDebug() && !layoutOk) {

        debugLog("imagesSyncEqual", "fingerprint ok but CMS gallery layout mismatch", {

          uniqueId: incomingFields?.["unique-id"],

          incomingMain: inMain,

          incomingSecond: inSecond,

          existingMain: galleryUrls(existingFieldData).length,

          existingSecond: secondGalleryUrls(existingFieldData).length,
